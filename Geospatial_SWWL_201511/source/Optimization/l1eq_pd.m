...
function [xp redo] = l1eq_pd(x0, A, At, b, pdtol, pdmaxiter, cgtol, cgmaxiter)

redo =0;
largescale = isa(A,'function_handle');

if (nargin < 5), pdtol = 1e-128;  end       % 100 -8
if (nargin < 6), pdmaxiter = 10000;  end    % 100 500
if (nargin < 7), cgtol = 1e-128;  end       % 100 -8
if (nargin < 8), cgmaxiter = 10000;  end    % 100 500
...
%   disp(sprintf('%8.3e',fu1p));
%     disp(sprintf('%8.3e',lamu1p));
%     disp(sprintf('%8.3e',fu2p(1)));
%       disp(sprintf('%8.3e',lamu2p(1)));
    if (isnan(fu1p(1))==1)
%       disp('fulp now is nan');
      xp = x;
      return
    end
...
%   disp(sprintf('%8.3e',sdg));
         % 100 -128

    
  sdg = -(fu1'*lamu1 + fu2'*lamu2);
%   disp(sprintf('%8.3e',fu1));
%     disp(sprintf('%8.3e',lamu1));
%     disp(sprintf('%8.3e',fu2));
%       disp(sprintf('%8.3e',lamu2));
...
%   done = (sdg < pdtol) | (pditer >= pdmaxiter); 
%   done = (sdg < pdtol) || (pditer >= pdmaxiter) | (tau==NaN);  
    done = (sdg < pdtol) | (pditer >= pdmaxiter); 
